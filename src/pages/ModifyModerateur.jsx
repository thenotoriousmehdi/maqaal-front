...
export const ModifyModerateur = () => {
  const moderateur ={
  username:"kahinasd",nom:"SAADAOUI Kahina",email:"kk_saadaoui@esi.dz",motdepasse:"123456",
  }
  ...
  const handleSubmit =()=>{
    if(mdp!== mdp1 || (mdp.length<8  && mdp1.length<8 && mdp.length!==0 && mdp1.length!==0 && !email.includes("@"))){
      setError(true)
    }else{ 
      setError(false)
      setSubmitted(true)
...
<Headroom>
  <Navbar />{" "}
      ...
      <div className="flex  xl:gap-14   gap-[10%]   items-center ">
          ...
          <button onClick={()=>(navigate(-1))} className="  border border-primary rounded-tl-3xl rounded-b-3xl lg:ml-28 ml-6  p-3  hover:bg-slate-700 text-white font-bold   transform transition-transform duration-200 ease-in-out hover:scale-110 focus:outline-none focus:ring-2 focus:ring-blue-800 focus:ring-opacity-75">
        ...
        <h1 className="font-title  font-extrabold my-14 text-xl sm:text-3xl md:text-5xl text-primary ">
          {" "}
          Modifer modérateur , <span className=" text-rosee "> { moderateur.nom}</span>
        </h1>
      </div>
    ...
    {error &&  <span className=" text-red-700 font-body text-xl font-bold my-2"> Erreur dans le mot de passe </span>}
            
            <button onClick={handleSubmit} className=" font-bold  w-36 h-16  text-white my-3 rounded-xl font-body text-xl bg-primary "> {submitted? "terminé !" : "terminer" }  </button>
           </div>
    </>
  );
};
